  ...
  return Object.keys(obj).reduce((previous, key) => {
    ...
    if (key === 'guide' && value) {
      ...
      Object.keys(value).forEach((item) => {
        ...
        } else if (Array.isArray(value[item])) {
          const array = value[item];

          Object.keys(array).forEach((arrayItem) => {
            if (typeof array[arrayItem] === 'string' && array[arrayItem].indexOf('!file=') !== -1) {
              const filepath = array[arrayItem].replace('!file=', '');
              array[arrayItem] = loadFile(path.resolve(dirpath, filepath));
            } else if (Array.isArray(array[arrayItem])) {
              array[arrayItem] = transformData(array[arrayItem], dirpath);
            } else if (array.hasOwnProperty(arrayItem) && typeof array[arrayItem] === 'object') {
              array[arrayItem] = transformData(array[arrayItem], dirpath);
            }
