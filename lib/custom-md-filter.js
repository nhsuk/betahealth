...
function customMdFilter(marker, name, hasParams = false, viewsPath = './app/views/_md-filters') {
  const contentPlaceholderPattern = /<!--\s*?content\s*?-->/;
  const filterNamePattern = new RegExp(`^${name}\\s+(.*)$`);
  const template = fs.readFileSync(`${path.join(config.root, viewsPath)}/${name}.nunjucks`, 'utf-8');
