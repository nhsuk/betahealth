...
function transformData(obj, dirpath) {
  return Object.keys(obj).reduce((previous, key) => {
    ...
    if (key === 'guide' && value) {
      ...
      Object.keys(guide.pages).forEach((page) => {
        const manifestPath = path.resolve(dirpath, '..', guide.pages[page], 'manifest.json');
        const pageManifest = loadFile(manifestPath);
        const pageObj = JSON.parse(pageManifest);

        pageObj.slug = guide.pages[page];

        guide.pages[page] = pageObj;
      });
